...
from data.flow_sampler import FlowSampler
...
class DINOTracker():
    def __init__(self, args):
        ...
        # self.flows = torch.load(self.flows_path, map_location=device)
        # self.valid_mask = torch.load(self.valid_mask_path, map_location=device)
    ...
    def set_paths(self, data_path):
        ...
        self.flows_path = config_paths['flows_path']
        self.valid_mask_path = config_paths['valid_mask_path']
    ...
    def load_trajectories(self):
        ...
        assert os.path.exists(self.fg_trajectories_path), "trajectory files don't exist"
    ...
    def get_sampler(self):        
        ...
        # train_sampler = FlowSampler(
        #     flows=self.flows,
        #     valid_mask=self.valid_mask,
        #     range_normalizer=self.range_normalizer,
        #     dst_range=(-1,1),
        #     batch_size=self.config["train_batch_size"],
        #     max_steps=5,  # or self.config["whatever"]
        #     device=device
        # )
    ...
    def update_losses(self, loss_total, loss_of, loss_emb_norm_reg, loss_angle_reg, loss_cyc):
    ...
    def log_losses(self, i, log_interval=100):
        ...
        loss_str = f"loss_of: {loss_of:.4f}, loss_emb_norm_reg: {loss_emb_norm_reg:.4f}, loss_angle_reg: {loss_angle_reg:.4f}"
    ...
    def train(self):
        ...
        for i in tqdm(range(self.init_iter, total_iterations)):
            ...
            loss +=  self.config["lambda_emb_norm"] * loss_emb_norm_reg + self.config["lambda_angle"] * loss_angle_reg
            loss.backward()
            scheduler.step()
            optimizer.step()
            
            # logging losses
            self.update_losses(loss.item(), tracking_loss.item(),
